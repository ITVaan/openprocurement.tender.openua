...
class TenderUaAwardContractResource(TenderAwardContractResource):
    ...
    def patch(self):
        ...
        if data['value']:
            ...
            if data['value']['amount'] > max_sum:
                self.request.errors.add(
                    'body', 'data', 'Value amount should be less or equal to awarded amount ({})'.format(max_sum)
                )
                self.request.errors.status = 403
        ...
        if self.request.context.status != 'active' and 'status' in data and data['status'] == 'active':
            ...
            if not self.award_valid(self.request, self.request.context.awardID):
                return

            for awardID in contract.get('additionalAwardIDs'):
                if not self.award_valid(self.request, awardID, additional=True):
                    return
