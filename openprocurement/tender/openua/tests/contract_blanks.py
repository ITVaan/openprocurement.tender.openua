...
def not_found(self):
    ...
    response = self.app.post('/tenders/{}/contracts/some_id/documents'.format(self.tender_id), status=404,
    ...
    response = self.app.put('/tenders/{}/contracts/some_id/documents/some_id'.format(self.tender_id), status=404,
                            upload_files=[
...
def try_merge_not_real_award_2(self):
    ...
    self.assertEqual(response.json['errors'],
                     [
                         {
                             u"location": u"body",
                             u"name": u"additionalAwardIDs",
                             u"description": [u"id must be one of award id"]
                         }
                     ])
...
def merge_two_contracts_2(self):
...
def standstill_period_2(self):
    ...
    response = self.app.patch_json("/tenders/{}/contracts/{}?acc_token={}".format(
        ...
        status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertIn(u"Can't sign contract before stand-still additional awards period end",
...
def additional_awards_dateSigned_2(self):
...
def activate_contract_with_complaint_2(self):
    ...
    response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(
        self.tender_id, second_contract['awardID'], self.initial_bids_tokens[self.initial_bids[1]['id']]),
        {'data': {
    ...
    response = self.app.patch_json(
        ...
        {"data": {"satisfied": True, "status": "cancelled", "cancellationReason": "Took answer"}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
...
def merge_two_contracts_with_different_suppliers_id_2(self):
...
def activate_contract_with_complaint_3(self):
    ...
    response = self.app.post_json('/tenders/{}/awards/{}/complaints?acc_token={}'.format(
        self.tender_id, second_contract['awardID'], self.initial_bids_tokens[self.initial_bids[0]['id']]),
        {'data': {
            'title': 'complaint title',
            'description': 'complaint description',
            'author': test_organization,
    ...
    response = self.app.post_json(
        '/tenders/{}/awards/{}/complaints?acc_token={}'.format(
            self.tender_id, third_contract['awardID'], self.initial_bids_tokens[self.initial_bids[1]['id']]),
        {'data': {
            'title': 'complaint title',
            'description': 'complaint description',
            'author': test_organization,
...
def activate_contract_with_complaint_4(self):
    ...
    response = self.app.post_json(
        '/tenders/{}/awards/{}/complaints?acc_token={}'.format(
            self.tender_id, fourth_contract['awardID'], self.initial_bids_tokens[self.initial_bids[0]['id']]),
        {'data': {
            'title': 'complaint title',
            'description': 'complaint description',
            'author': test_organization,
    ...
    print('first = {}'.format(first_contract['status']))
    print('second = {}'.format(second_contract['status']))
    print('third = {}'.format(third_contract['status']))
    print('fourth = {}'.format(fourth_contract['status']))
    print('first id = {}'.format(first_contract['id']))
    print('second id = {}'.format(second_contract['id']))
    print('third id = {}'.format(third_contract['id']))
    print('fourth id = {}'.format(fourth_contract['id']))
    # print('first additionalAwardIDs = {}'.format(first_contract['additionalAwardIDs']))
    # print('second mergedInto = {}'.format(second_contract['mergedInto']))
    print('third additionalAwardIDs = {}'.format(third_contract['additionalAwardIDs']))
    print('fourth mergedInto = {}'.format(fourth_contract['mergedInto']))
                     ...
                     [
                         {
                             u"location": u"body",
                             u"name": u"data",
                             u"description": u"Can't sign contract before reviewing all additional complaints"
                         }
                     ])
